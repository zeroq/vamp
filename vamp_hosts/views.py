...
def view_host(request, hostid):
    ...
    context['waiting_exception'] = Finding.objects.filter(host_id=hostid, status=6, request_type='exception').count()
    context['waiting_ignore'] = Finding.objects.filter(host_id=hostid, status=6, request_type='ignore').count()
    context['have_exception'] = Exceptions.objects.filter(host=host_obj, approved=True).count()
    activeTab = ''
...
def reset_exception_request(request, hostid, findingid):
    ...
    finding_obj.request_type = ''
    finding_obj.date_remediated = None
    finding_obj.save()
    return redirect(reverse('hosts:view_host', kwargs={'hostid': hostid}))

@login_required
def reset_ignore_request(request, hostid, findingid):
    hostid = int(hostid)
    findingid = int(findingid)
    context = {}
    try:
    ...
    # fetch ignore object
    try:
        i_obj = Ignore.objects.get(finding=finding_obj, host=host_obj)
    except Exceptions.DoesNotExist:
        messages.error(reuqest, 'Exception not found!')
        return render(request, 'vamp_hosts/list_hosts.html', context)
    # delete exceptions request
