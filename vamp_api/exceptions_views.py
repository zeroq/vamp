...
from vamp_api.serializers import ExceptionSerializer, IgnoreSerializer
...
def list_open_ignore(request, format=None):
    """List all open ignore requests
    """
    paginator = CustomPaginator()
    queryset = Ignore.objects.filter(approved=False)
    ...
    serializer = IgnoreSerializer(instance=exceptions, many=True)
    return paginator.get_paginated_response(serializer.data)

@api_view(['GET'])
@authentication_classes((SessionAuthentication, ))
@permission_classes((IsAuthenticated,))
def list_granted_ignore(request, format=None):
    """List all granted ignore requests
    """
    paginator = CustomPaginator()
    queryset = Ignore.objects.filter(approved=True)
    order_by_column, order_direction = get_ordering_vars(request.query_params,
                                                         default_column='last_update',
                                                         default_direction='-')
    if order_by_column:
